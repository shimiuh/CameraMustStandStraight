...
package com.shimi.camera.must.stand.straight;
...
import com.shimi.camera.must.stand.straight.R;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener,
            ...
            switch (v.getId()) {
                case R.id.take_picture:
                    if(canTakePhoto()){
                        ...
                        Toast.makeText(MainActivity.this, R.string.camera_is_not_straight,Toast.LENGTH_SHORT).show();
                    }
    ...
    private void updateUiOnMovement(SensorEvent event) {
        float x = event.values[0];
        float y = event.values[1];
        float z = event.values[2];
        int rotationVal = (int) (x * 8);
        mCameraView.setRotation(rotationVal);
        mCameraView.setRotationX(z * 2);
        if(canTakePhoto()){
            if(mCaptureFab.getAlpha() != 1) {
                mCaptureFab.animate().cancel();
                mCaptureFab.animate().alpha(1).start();
            }
        }else{
            if(mCaptureFab.getAlpha() != 0) {
                mCaptureFab.animate().cancel();
                mCaptureFab.animate().alpha(0f).start();
            }
        }
    }

    private boolean canTakePhoto() {
        return Math.abs(mCameraView.getRotation()) < 1 && Math.abs(mCameraView.getRotationX()) < 1;
    }
    ...
    protected void onPause() {
        //when on Pause, release camera in order to be used from other applications and unregister SensorManager
        mCameraView.stop();
